...
import { Home, Car, DollarSign, Plus, X, Search, Trash2, ChevronRight, Bell, AlertTriangle, AlertCircle, CheckCircle, ArrowLeft, Download, Upload, FileText, Shield, Wrench, RefreshCw, TrendingUp, MoreHorizontal, List, Edit, Camera, Cloud, CloudOff } from "lucide-react"
...
const DOC_CATS = [['doc_registration','📋 Registration / Istimara'],['doc_insurance','🛡️ Insurance Document'],['doc_inspection','🔍 Inspection Certificate'],['doc_other','📎 Other Documents']]
...
const DEMO = {
  ownerName: 'Ahmed',
  cars: [
    { id:'c1', nickname:'Farida', brand:'FORD', model:'Sedan', year:2018, plateNumber:'815266', color:'White', vin:'', currentMileage:161000, status:'rented', purchaseDate:'2020-01-15', purchasePrice:45000, estimatedValue:28000, notes:'', registrationExpiry:'2026-08-15', insuranceExpiry:'2026-07-04', inspectionExpiry:'2026-09-20', roadPermitExpiry:'2026-12-01', warrantyExpiry:'', lastOilChangeMileage:158000, nextOilChangeMileage:163000, lastOilChangeDate:'2026-04-28', oilType:'5W-30', garage:'Al Mana Garage', oilChangeCost:120, createdAt:new Date().toISOString() },
    { id:'c2', nickname:'MG Silver', brand:'MG', model:'SUV', year:2023, plateNumber:'916941', color:'Silver', vin:'', currentMileage:54000, status:'available', purchaseDate:'2023-03-15', purchasePrice:75000, estimatedValue:65000, notes:'', registrationExpiry:'2026-12-01', insuranceExpiry:'2026-07-03', inspectionExpiry:'2026-06-24', roadPermitExpiry:'2027-03-01', warrantyExpiry:'2028-03-15', lastOilChangeMileage:52000, nextOilChangeMileage:57000, lastOilChangeDate:'2026-05-28', oilType:'0W-20', garage:'MG Service Center', oilChangeCost:180, createdAt:new Date().toISOString() },
    { id:'c3', nickname:'Chery Black', brand:'CHERY', model:'Sedan', year:2026, plateNumber:'3', color:'Black', vin:'', currentMileage:5000, status:'available', purchaseDate:'2025-12-01', purchasePrice:90000, estimatedValue:88000, notes:'New car – just purchased', registrationExpiry:'2027-12-01', insuranceExpiry:'2027-06-01', inspectionExpiry:'2027-06-01', roadPermitExpiry:'2027-12-01', warrantyExpiry:'2029-12-01', lastOilChangeMileage:0, nextOilChangeMileage:5000, lastOilChangeDate:'', oilType:'5W-30', garage:'', oilChangeCost:0, createdAt:new Date().toISOString() },
  ],
  rentals: [
    { id:'r1', carId:'c1', customerName:'REEM', customerPhone:'+974 5555 1234', customerIdNumber:'QA12345678', startDate:'2026-01-06', endDate:'2026-01-11', actualReturnDate:null, rateType:'daily', rateAmount:900, deposit:1000, paymentReceived:4500, startMileage:156000, endMileage:null, fuelOut:80, fuelIn:null, conditionOut:'Good condition', conditionIn:'', status:'active', notes:'', createdAt:new Date().toISOString() },
  ],
  accidents: [],
  expenses: [
    { id:'e1', carId:'c1', type:'maintenance', amount:120, date:'2026-04-28', description:'Oil change – Al Mana Garage', notes:'', createdAt:new Date().toISOString() },
    { id:'e2', carId:'c2', type:'insurance', amount:2500, date:'2025-07-03', description:'Annual insurance renewal', notes:'', createdAt:new Date().toISOString() },
    { id:'e3', carId:'c2', type:'maintenance', amount:180, date:'2026-05-28', description:'Oil change – MG Service Center', notes:'', createdAt:new Date().toISOString() },
  ],
  media: [],
}

// ── MEDIA STORAGE (localStorage) ──
const mediaCache = new Map()
const MEDIA_PFX = 'fm-media:'

async function saveMedia(id, dataUrl) {
  mediaCache.set(id, dataUrl)
  try { localStorage.setItem(MEDIA_PFX + id, dataUrl) } catch(e) {
    if (e.name === 'QuotaExceededError') alert('Storage full! Try deleting some photos in Settings.')
  }
}
async function loadMedia(id) {
  if (mediaCache.has(id)) return mediaCache.get(id)
  const v = localStorage.getItem(MEDIA_PFX + id)
  if (v) { mediaCache.set(id, v); return v }
  return null
}
async function deleteMedia(id) {
  mediaCache.delete(id)
  localStorage.removeItem(MEDIA_PFX + id)
}
...
function reduce(s, a) {
  switch(a.t) {
    ...
    case 'LOAD':      return { cars:[], rentals:[], accidents:[], expenses:[], media:[], ownerName:'Ahmed', ...a.p }
...
function Store({ children }) {
  const [state, dispatch] = useReducer(reduce, null, () => {
    try {
      const raw = localStorage.getItem('fleet-v2')
      if (raw) return { cars:[], rentals:[], accidents:[], expenses:[], media:[], ownerName:'Ahmed', ...JSON.parse(raw) }
    } catch(_) {}
    return DEMO
  })
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(() => { try { localStorage.setItem('fleet-v2', JSON.stringify(state)) } catch(e) {} }, 700)
  }, [state])
...
function PhotoPicker({ ownerType, ownerId, category }) {
  ...
  const handle = async e => {
    ...
    catch(err) { alert('Could not process photo – try another one.') }
    setBusy(false)
  }
  return (
    <button onClick={() => ref.current?.click()} disabled={busy} className="w-20 h-20 rounded-2xl border-2 border-dashed border-violet-200 bg-violet-50 flex flex-col items-center justify-center gap-1 active:bg-violet-100 flex-shrink-0 transition-colors">
...
function MediaSection({ title, ownerType, ownerId, category, single=false }) {
  const { s, d } = useStore()
  const items = s.media.filter(m => m.ownerType === ownerType && m.ownerId === ownerId && m.category === category)
...
function BottomNav({ tab, setTab }) {
  const NAV = [
    ...
    { id:'more',    Icon:MoreHorizontal,label:'More',   color:'text-gray-600',   bg:'bg-gray-100' },
  ]
  return (
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex flex-col justify-end" onClick={onClose}>
      <div className="bg-gray-50 rounded-t-3xl max-h-[93vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-5 py-3 flex-shrink-0">
          ...
          <button onClick={onClose} className="p-2 rounded-2xl bg-gray-100 active:bg-gray-200"><X size={15} className="text-gray-500" /></button>
        </div>
...
function CloudSync() {
  ...
  const push = async () => {
    if (!token) { setStatus({ ok:false, msg:'Enter your GitHub Personal Access Token first.' }); return }
    setBusy(true); setStatus({ ok:null, msg:'☁️ Saving to GitHub…' })
    try {
      const payload = JSON.stringify({
        description: '🚗 Fleet Manager Auto-Backup',
        public: false,
        files: { 'fleet-manager-data.json': { content: JSON.stringify(s) } }
      })
      ...
      const newId = data.id
      if (!gistId || gistId !== newId) { setGistId(newId); localStorage.setItem('fleet-gh-gist', newId) }
      localStorage.setItem('fleet-gh-token', token)
      const ts = new Date().toLocaleString('en-GB')
      setLastSync(ts); localStorage.setItem('fleet-gh-lastsync', ts)
      setStatus({ ok:true, msg:`✅ Saved! Gist ID: ${newId.slice(0,12)}…  — Bookmark this page; your data lives at that Gist.` })
    } catch(e) { setStatus({ ok:false, msg:`❌ ${e.message}` }) }
    setBusy(false)
  }

  const pull = async () => {
    if (!token) { setStatus({ ok:false, msg:'Enter your GitHub token.' }); return }
    if (!gistId) { setStatus({ ok:false, msg:'No Gist ID — push first, or paste an existing Gist ID.' }); return }
    ...
    try {
      ...
      const data = await res.json()
      const content = data.files['fleet-manager-data.json']?.content
      if (!content) throw new Error('fleet-manager-data.json not found in that Gist.')
      const parsed = JSON.parse(content)
      d({ t:'LOAD', p:parsed })
      const ts = new Date().toLocaleString('en-GB')
      setLastSync(ts); localStorage.setItem('fleet-gh-lastsync', ts)
      localStorage.setItem('fleet-gh-token', token)
      setStatus({ ok:true, msg:'✅ Data loaded from cloud! 🎉' })
    } catch(e) { setStatus({ ok:false, msg:`❌ ${e.message}` }) }
    setBusy(false)
  }
  ...
  return (
    <SCard title="☁️ Cloud Sync — GitHub Gist">
      <div className="mb-4 p-3.5 bg-indigo-50 rounded-2xl text-xs text-indigo-800 leading-relaxed">
        <p className="font-black mb-2">One-time setup (2 min):</p>
        <p>1. Open <strong>github.com</strong> → Settings → Developer settings → Personal access tokens → Fine-grained tokens</p>
        <p className="mt-1">2. Generate a token with <strong>Gist read/write</strong> permission</p>
        <p className="mt-1">3. Paste below → tap <strong>Push to Cloud</strong> → done! Your data is saved to a private Gist</p>
        <p className="mt-2 text-indigo-500">⚠️ Photos are stored on this device only. All car & rental records sync to the cloud.</p>
      </div>
      <Fld label="GitHub Personal Access Token">
        <div className="relative">
          <Inp value={token} onChange={e => setToken(e.target.value)} type={showToken ? 'text' : 'password'} placeholder="ghp_xxxxxxxxxxxxxxxxxxxx" className="pr-16" />
          <button onClick={() => setShowToken(v => !v)} className="absolute right-3 top-3 text-xs text-violet-500 font-black">{showToken ? 'Hide' : 'Show'}</button>
        </div>
      </Fld>
      <Fld label="Gist ID (auto-filled after first push)">
        <Inp value={gistId} onChange={e => setGistId(e.target.value)} placeholder="Paste existing Gist ID or leave empty" />
      </Fld>
      {lastSync && <p className="text-xs text-gray-400 mb-3">🕐 Last synced: {lastSync}</p>}
      {status && (
        <div className={`p-3 rounded-2xl text-xs font-semibold mb-4 leading-relaxed ${status.ok === true ? 'bg-emerald-50 text-emerald-700' : status.ok === false ? 'bg-rose-50 text-rose-700' : 'bg-blue-50 text-blue-700'}`}>
          {status.msg}
        </div>
      )}
      <div className="flex gap-2">
        <Btn onClick={push} disabled={busy} className="flex-1 flex items-center justify-center gap-1.5" size="md">
          <Upload size={14} />Push to Cloud
        </Btn>
        <Btn onClick={pull} v="outline" disabled={busy} className="flex-1 flex items-center justify-center gap-1.5" size="md">
          <Download size={14} />Pull from Cloud
        </Btn>
      </div>
      {gistId && <p className="text-[10px] text-gray-400 mt-3 text-center">Gist: <a href={`https://gist.github.com/${gistId}`} target="_blank" rel="noopener noreferrer" className="text-violet-500 underline">{gistId.slice(0,20)}…</a></p>}
    </SCard>
  )
}
...
function Dashboard({ nav }) {
  ...
  const monthExp = useMemo(() => { const m=new Date().getMonth(),y=new Date().getFullYear(); return s.expenses.filter(e=>new Date(e.date).getMonth()===m&&new Date(e.date).getFullYear()===y).reduce((a,e)=>a+(e.amount||0),0) }, [s.expenses])
  ...
  const greet = h<6?'🌙 Up late?':h<12?'☀️ Good morning':h<17?'👋 Good afternoon':h<21?'🌅 Good evening':'🌙 Good evening'
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-violet-50/40 pb-28">
      <div className="px-5 pt-12 pb-4 bg-white">
        <div className="flex items-center justify-between mb-1">
          <div>
            <p className="text-gray-400 text-sm font-semibold">{greet}, {s.ownerName||'Ahmed'}!</p>
            <h1 className="text-3xl font-black text-gray-900 tracking-tight">Summary</h1>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={()=>nav('renewals',{})} className="relative p-2.5 bg-gray-50 rounded-2xl active:bg-gray-100">
              <Bell size={18} className="text-gray-500" />
              {alerts.length>0&&<span className="absolute -top-1 -right-1 w-5 h-5 bg-rose-500 rounded-full text-[9px] font-black text-white flex items-center justify-center shadow-sm">{alerts.length}</span>}
            </button>
        ...
        <p className="text-xs text-gray-300 font-semibold">{new Date().toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</p>
      </div>
      <div className="px-4 pt-4 grid grid-cols-2 gap-3">
        ...
        <StatTile emoji="💰" label="This Month" value={fmt(monthIncome)} sub={monthExp?`${fmt(monthExp)} spent`:'No expenses yet'} bars={barsFrom(last6)} gradient="from-amber-400 to-orange-500" onClick={()=>nav('finance',{})} />
        <StatTile emoji={alerts.length?'🔔':'✅'} label="Alerts" value={alerts.length||'All clear!'} sub={alerts.length?alerts[0].text:'Everything looks great 💪'} bars={barsFrom([alerts.filter(a=>a.type==='error').length||0,alerts.filter(a=>a.type==='warn').length||0,1])} gradient={alerts.length?'from-rose-400 to-pink-500':'from-emerald-400 to-teal-500'} onClick={()=>nav('renewals',{})} />
      </div>
      <div className="px-4 mt-4">
        <SCard title="✨ Quick Actions">
          <div className="grid grid-cols-3 gap-3">
            {[{em:'🚗',label:'Add Car',fn:()=>nav('car-form',{}),bg:'bg-violet-50'},{em:'🔑',label:'New Rental',fn:()=>nav('rental-form',{}),bg:'bg-blue-50'},{em:'🔔',label:'Renewals',fn:()=>nav('renewals',{}),bg:'bg-amber-50'},{em:'💵',label:'Add Expense',fn:()=>nav('expense-form',{}),bg:'bg-emerald-50'},{em:'🚨',label:'Accident',fn:()=>nav('accidents',{}),bg:'bg-rose-50'},{em:'🛢️',label:'Oil Change',fn:()=>nav('maintenance',{}),bg:'bg-cyan-50'}].map(({em,label,fn,bg})=>(
              <button key={label} onClick={fn} className={`flex flex-col items-center gap-2 p-3 ${bg} rounded-2xl active:scale-95 transition-transform`}>
                ...
                <span className="text-[11px] font-black text-gray-600 text-center leading-tight">{label}</span>
              </button>
            ))}
          </div>
        </SCard>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 pb-28">
      ...
      <div className="px-4 py-3 space-y-3">
        <div className="relative"><Search size={15} className="absolute left-3.5 top-3.5 text-gray-300"/><Inp value={q} onChange={e=>setQ(e.target.value)} placeholder="Search name, brand, plate…" className="pl-10"/></div>
        <div className="flex gap-2 overflow-x-auto pb-1 no-scroll">{[['all','All 🚗'],['available','✅ Available'],['rented','🔑 Rented'],['maintenance','🔧 Workshop'],['accident','🚨 Accident'],['sold','📦 Sold']].map(([v,l])=><button key={v} onClick={()=>setF(v)} className={`px-3 py-1.5 rounded-full text-xs font-black whitespace-nowrap flex-shrink-0 transition-all ${f===v?'bg-violet-600 text-white shadow-sm shadow-violet-200':'bg-white text-gray-400 border border-gray-200'}`}>{l}</button>)}</div>
        {list.length===0&&<div className="text-center py-16"><div className="text-5xl mb-3">🚗</div><p className="font-black text-gray-400">No cars found</p><p className="text-sm text-gray-300 mt-1">Add your first ride to get started!</p></div>}
...
function CarCard({ car, onClick }) {
  const { s } = useStore(); const photo=s.media.find(m=>m.ownerType==='car'&&m.ownerId===car.id&&m.category==='exterior'); const activeRent=s.rentals.find(r=>r.carId===car.id&&r.status==='active'); const oilKm=(car.nextOilChangeMileage||0)-(car.currentMileage||0); const insD=daysUntil(car.insuranceExpiry); const hasAlert=oilKm<=500||(insD!==null&&insD<=30); const accent=STATUS_ACCENT[car.status]||'from-gray-300 to-gray-400'
  return (
    <button onClick={onClick} className="w-full bg-white rounded-3xl shadow-sm overflow-hidden text-left active:scale-[0.98] transition-transform mb-3">
      ...
      <div className="p-4">
        <div className="flex gap-3 mb-3"><CarThumb mediaId={photo?.id} size="w-14 h-14" iconSize={22}/><div className="flex-1 min-w-0 flex justify-between items-start gap-2"><div className="min-w-0"><div className="flex items-center gap-1.5"><span className="font-black text-gray-900 text-base truncate">{car.nickname}</span>{hasAlert&&<span>⚠️</span>}</div><div className="text-xs text-gray-400 mt-0.5">{car.brand} {car.model} · {car.year}</div></div><Badge status={car.status} map={CAR_STATUS}/></div></div>
        <div className="grid grid-cols-3 gap-0 text-xs bg-gray-50 rounded-2xl overflow-hidden"><div className="text-center p-2.5"><div className="text-gray-400 text-[10px] font-black mb-0.5">PLATE</div><div className="font-black text-gray-700">{car.plateNumber}</div></div><div className="text-center p-2.5 border-x border-gray-100"><div className="text-gray-400 text-[10px] font-black mb-0.5">KM</div><div className="font-black text-gray-700">{(car.currentMileage||0).toLocaleString()}</div></div><div className="text-center p-2.5"><div className="text-gray-400 text-[10px] font-black mb-0.5">OIL</div><div className={`font-black ${oilKm<=0?'text-rose-600':oilKm<=500?'text-amber-500':'text-emerald-600'}`}>{oilKm<=0?'⚠️ Due':`${oilKm.toLocaleString()}km`}</div></div></div>
...
function CarDetail({ carId, nav }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-violet-50/30 pb-28">
      ...
      <div className="bg-white border-b border-gray-100 flex overflow-x-auto no-scroll">{[['info','ℹ️ Info'],['photos','📷 Photos'],['rentals','🔑 Rentals'],['docs','📋 Docs'],['finance','💰 Finance'],['acc','🚨 Accidents']].map(([v,l])=><button key={v} onClick={()=>setTab(v)} className={`px-4 py-3 text-xs font-black whitespace-nowrap border-b-2 transition-colors flex-shrink-0 ${tab===v?'border-violet-500 text-violet-600':'border-transparent text-gray-400'}`}>{l}</button>)}</div>
      <div className="px-4 py-4 space-y-3">
        {tab==='info'&&<><SCard title="🚗 Vehicle Info">{[['Brand',car.brand],['Model',car.model],['Year',car.year],['Color',car.color],['VIN',car.vin],['Purchase Date',fmtD(car.purchaseDate)],['Purchase Price',fmt(car.purchasePrice)],['Est. Value',fmt(car.estimatedValue)]].map(([l,v])=><InfoRow key={l} label={l} value={v}/>)}</SCard><SCard title="🛢️ Oil Change"><InfoRow label="Current KM" value={`${(car.currentMileage||0).toLocaleString()} km`}/><InfoRow label="Last Change" value={fmtD(car.lastOilChangeDate)}/><InfoRow label="Last KM" value={`${(car.lastOilChangeMileage||0).toLocaleString()} km`}/><InfoRow label="Next at" value={`${(car.nextOilChangeMileage||0).toLocaleString()} km`}/><InfoRow label="Remaining" value={oilKm<=0?'🚨 OVERDUE!':oilKm<=500?`⚠️ ${oilKm} km`:`✅ ${oilKm.toLocaleString()} km`} danger={oilKm<=0}/><InfoRow label="Oil Type" value={car.oilType}/><InfoRow label="Garage" value={car.garage}/></SCard>{car.notes&&<SCard title="📝 Notes"><p className="text-sm text-gray-600">{car.notes}</p></SCard>}<Btn onClick={()=>nav('car-form',{id:carId})} v="outline" className="w-full flex items-center justify-center gap-2"><Edit size={15}/>Edit Car Details</Btn></>}
        ...
        {tab==='rentals'&&<><Btn onClick={()=>nav('rental-form',{carId})} className="w-full flex items-center justify-center gap-2"><Plus size={15}/>🔑 New Rental</Btn>{carRentals.length===0&&<div className="text-center py-10"><div className="text-4xl mb-2">📋</div><p className="font-black text-gray-300">No rentals yet</p></div>}{carRentals.sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)).map(r=><button key={r.id} onClick={()=>nav('rental-detail',{id:r.id})} className="w-full bg-white rounded-3xl p-4 shadow-sm text-left active:scale-[0.98] transition-transform"><div className="flex justify-between items-start"><div><div className="font-bold text-gray-800">{r.customerName}</div><div className="text-xs text-gray-400">{fmtD(r.startDate)} → {fmtD(r.endDate)}</div></div><Badge status={r.status} map={RENT_STATUS}/></div><div className="flex justify-between text-xs text-gray-400 mt-2"><span>{fmt(r.rateAmount)}/{r.rateType}</span><span>💰 {fmt(r.paymentReceived||0)}</span></div></button>)}</>}
        ...
        {tab==='finance'&&<><div className="grid grid-cols-3 gap-2"><div className="bg-emerald-50 rounded-2xl p-3 text-center"><div className="text-base font-black text-emerald-700">{fmt(totalIncome)}</div><div className="text-xs text-emerald-500">💰 Income</div></div><div className="bg-rose-50 rounded-2xl p-3 text-center"><div className="text-base font-black text-rose-600">{fmt(totalExp)}</div><div className="text-xs text-rose-400">💸 Expenses</div></div><div className={`${totalIncome-totalExp>=0?'bg-violet-50':'bg-amber-50'} rounded-2xl p-3 text-center`}><div className={`text-base font-black ${totalIncome-totalExp>=0?'text-violet-700':'text-amber-700'}`}>{fmt(totalIncome-totalExp)}</div><div className="text-xs text-gray-400">📈 Net</div></div></div><Btn onClick={()=>nav('expense-form',{carId})} v="outline" className="w-full flex items-center justify-center gap-2"><Plus size={15}/>Add Expense</Btn>{carExp.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(e=><div key={e.id} className="bg-white rounded-2xl p-3 shadow-sm flex justify-between items-center"><div><div className="text-sm font-bold text-gray-700 capitalize">{e.type}</div><div className="text-xs text-gray-400">{e.description} · {fmtD(e.date)}</div></div><span className="text-rose-600 font-black text-sm">{fmt(e.amount)}</span></div>)}</>}
        {tab==='acc'&&<><Btn onClick={()=>nav('accidents',{openForCar:carId})} className="w-full flex items-center justify-center gap-2"><Plus size={15}/>🚨 Record Accident</Btn>{carAcc.length===0&&<div className="text-center py-10"><div className="text-4xl mb-2">🙏</div><p className="font-black text-gray-300">No accidents!</p></div>}{carAcc.map(a=><div key={a.id} className="bg-white rounded-3xl p-4 shadow-sm"><div className="flex justify-between items-start mb-1"><div><div className="font-bold text-gray-800 text-sm">{fmtD(a.date)}</div><div className="text-xs text-gray-500 mt-0.5">{a.description}</div></div><Badge status={a.status} map={ACC_STATUS}/></div><div className="text-xs text-gray-400 mt-2">Est: {fmt(a.estimatedCost||0)} · Actual: {fmt(a.actualCost||0)}</div></div>)}</>}
      </div>
...
// ════════════ CAR FORM ════════════
function CarForm({ carId, nav }) {
  const { s, d } = useStore(); const existing=carId?s.cars.find(c=>c.id===carId):null
  const [form,setForm]=useState(existing||{id:uid(),nickname:'',brand:'',model:'',year:new Date().getFullYear(),plateNumber:'',color:'',vin:'',currentMileage:'',status:'available',purchaseDate:'',purchasePrice:'',estimatedValue:'',notes:'',registrationExpiry:'',insuranceExpiry:'',inspectionExpiry:'',roadPermitExpiry:'',warrantyExpiry:'',lastOilChangeMileage:'',nextOilChangeMileage:'',lastOilChangeDate:'',oilType:'5W-30',garage:'',oilChangeCost:'',createdAt:new Date().toISOString()})
  const set=(k,v)=>setForm(f=>({...f,[k]:v}))
  const save=()=>{ if(!form.nickname||!form.brand){alert('Please enter car name and brand');return}; const p={...form,year:+form.year||2020,currentMileage:+form.currentMileage||0,purchasePrice:+form.purchasePrice||0,estimatedValue:+form.estimatedValue||0,lastOilChangeMileage:+form.lastOilChangeMileage||0,nextOilChangeMileage:+form.nextOilChangeMileage||5000,oilChangeCost:+form.oilChangeCost||0}; d({t:existing?'UPD_CAR':'ADD_CAR',p}); nav('car-detail',{id:p.id}) }
  return (
    <div className="min-h-screen bg-gray-50 pb-28">
      <Header title={existing?'✏️ Edit Car':'🚗 Add Car'} onBack={()=>nav(existing?'car-detail':'cars',existing?{id:carId}:{})}/>
      <div className="px-4 py-4 space-y-4">
        <SCard title="🚗 Basic Info"><Fld label="Nickname *"><Inp value={form.nickname} onChange={e=>set('nickname',e.target.value)} placeholder="Farida, Black MG…"/></Fld><Fld label="Brand *"><Inp value={form.brand} onChange={e=>set('brand',e.target.value)} placeholder="FORD, MG, CHERY…"/></Fld><div className="grid grid-cols-2 gap-3"><Fld label="Model"><Inp value={form.model} onChange={e=>set('model',e.target.value)}/></Fld><Fld label="Year"><Inp type="number" value={form.year} onChange={e=>set('year',e.target.value)}/></Fld></div><div className="grid grid-cols-2 gap-3"><Fld label="Plate"><Inp value={form.plateNumber} onChange={e=>set('plateNumber',e.target.value)}/></Fld><Fld label="Color"><Inp value={form.color} onChange={e=>set('color',e.target.value)}/></Fld></div><Fld label="VIN"><Inp value={form.vin} onChange={e=>set('vin',e.target.value)}/></Fld><Fld label="Status"><Sel value={form.status} onChange={e=>set('status',e.target.value)}>{Object.entries(CAR_STATUS).map(([v,{label,emoji}])=><option key={v} value={v}>{emoji} {label}</option>)}</Sel></Fld></SCard>
...
function RentalsScreen({ nav }) {
  ...
  const list=useMemo(()=>{ let r=s.rentals; if(f!=='all')r=r.filter(x=>x.status===f); if(q)r=r.filter(x=>x.customerName.toLowerCase().includes(q.toLowerCase())||(s.cars.find(c=>c.id===x.carId)?.plateNumber||'').includes(q)); return r.sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)) },[s.rentals,s.cars,f,q])
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-teal-50/30 pb-28">
      ...
      <div className="px-4 py-3 space-y-3">
        <div className="relative"><Search size={15} className="absolute left-3.5 top-3.5 text-gray-300"/><Inp value={q} onChange={e=>setQ(e.target.value)} placeholder="Search customer or plate…" className="pl-10"/></div>
        ...
        {list.length===0&&<div className="text-center py-16"><div className="text-5xl mb-3">📋</div><p className="font-black text-gray-400">No rentals here</p></div>}
        {list.map(r=>{ const car=s.cars.find(c=>c.id===r.carId); const photo=s.media.find(m=>m.ownerType==='car'&&m.ownerId===r.carId&&m.category==='exterior'); const dur=calcDur(r.startDate,r.endDate); const total=r.rateAmount*dur; const dd=daysUntil(r.endDate); const isLate=r.status==='active'&&dd<0; return <button key={r.id} onClick={()=>nav('rental-detail',{id:r.id})} className="w-full bg-white rounded-3xl p-4 shadow-sm text-left active:scale-[0.98] transition-transform mb-3"><div className="flex items-start gap-3 mb-3"><CarThumb mediaId={photo?.id} size="w-11 h-11" iconSize={16} rounded="rounded-xl"/><div className="flex-1 min-w-0 flex justify-between items-start gap-2"><div className="min-w-0"><div className="font-black text-gray-900 truncate">{r.customerName}</div><div className="text-xs text-gray-400 truncate">{car?.nickname} · {car?.plateNumber}</div></div><Badge status={r.status} map={RENT_STATUS}/></div></div><div className="grid grid-cols-3 gap-0 text-xs bg-gray-50 rounded-2xl overflow-hidden"><div className="p-2.5 text-center"><div className="text-gray-400 text-[10px] font-black mb-0.5">START</div><div className="font-bold text-gray-700">{fmtD(r.startDate)}</div></div><div className="p-2.5 text-center border-x border-gray-100"><div className="text-gray-400 text-[10px] font-black mb-0.5">END</div><div className={`font-bold ${isLate?'text-rose-600':'text-gray-700'}`}>{fmtD(r.endDate)}</div></div><div className="p-2.5 text-center"><div className="text-gray-400 text-[10px] font-black mb-0.5">TOTAL</div><div className="font-bold text-gray-700">{fmt(total)}</div></div></div>{isLate&&<div className="mt-2 text-xs text-rose-500 flex items-center gap-1 font-black bg-rose-50 rounded-xl px-3 py-2"><AlertCircle size={12}/>🚨 {Math.abs(dd)} days overdue!</div>}</button> })}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-28">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <SCard title="💰 Payment"><div className="grid grid-cols-2 gap-3"><Fld label="Rate Type"><Sel value={form.rateType} onChange={e=>set('rateType',e.target.value)}><option value="daily">📅 Daily</option><option value="weekly">📆 Weekly</option><option value="monthly">🗓️ Monthly</option></Sel></Fld><Fld label="Rate (QAR)"><Inp type="number" value={form.rateAmount} onChange={e=>set('rateAmount',e.target.value)}/></Fld></div><div className="grid grid-cols-2 gap-3"><Fld label="Deposit (QAR)"><Inp type="number" value={form.deposit} onChange={e=>set('deposit',e.target.value)}/></Fld><Fld label="Paid (QAR)"><Inp type="number" value={form.paymentReceived} onChange={e=>set('paymentReceived',e.target.value)}/></Fld></div>
        ...
        <div className="bg-violet-50 border border-violet-100 rounded-2xl p-3.5 text-xs text-violet-700 font-semibold">📎 After saving, attach the driving license, customer ID, contract & photos on the rental page.</div>
...
function RentalDetail({ rentalId, nav }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-28">
      ...
      {showCheckin&&<CheckInModal car={car} rental={rental} onClose={()=>setShowCheckin(false)} onSave={checkIn}/>}
      {confirmDel&&<Confirm msg={`Delete rental for ${rental.customerName}?`} onOk={del} onCancel={()=>setConfirmDel(false)}/>}
    </div>
  )
}
function CheckInModal({ car, rental, onClose, onSave }) {
  const [form,setForm]=useState({actualReturnDate:today(),endMileage:'',fuelIn:80,conditionIn:''}); const set=(k,v)=>setForm(f=>({...f,[k]:v}))
  return (<Modal title={`✅ Check In – ${car?.nickname}`} onClose={onClose} footer={<Btn onClick={()=>onSave(form)} v="success" className="w-full">✅ Confirm Return</Btn>}><Fld label="Return Date"><Inp type="date" value={form.actualReturnDate} onChange={e=>set('actualReturnDate',e.target.value)}/></Fld><Fld label="Return Mileage (km)"><Inp type="number" value={form.endMileage} onChange={e=>set('endMileage',e.target.value)} placeholder={car?.currentMileage}/></Fld><Fld label={`⛽ Fuel Level In: ${form.fuelIn}%`}><input type="range" min={0} max={100} step={10} value={form.fuelIn} onChange={e=>set('fuelIn',+e.target.value)} className="w-full accent-emerald-500"/></Fld><Fld label="Condition on Return"><Txa value={form.conditionIn} onChange={e=>set('conditionIn',e.target.value)} placeholder="Any damage or issues?"/></Fld><p className="text-xs text-gray-400 -mt-1">Attach return photos from the rental page after saving.</p></Modal>)
}
...
function MaintenanceScreen({ nav }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-cyan-50/30 pb-28">
      ...
      <div className="px-4 py-4 space-y-4">
        <div className="bg-gradient-to-br from-amber-400 to-orange-500 text-white rounded-3xl p-4 shadow-lg"><div className="text-2xl mb-1">🔧</div><div className="font-black text-lg">Oil Change Tracker</div><div className="text-white/70 text-sm">Stay on top of every car's service schedule</div></div>
        {s.cars.filter(c=>c.status!=='sold').map(car=>{ const km=(car.nextOilChangeMileage||0)-(car.currentMileage||0); const pct=Math.min(100,Math.max(0,(1-km/5000)*100)); const over=km<=0; const soon=km>0&&km<=500; return (
          <div key={car.id} className="bg-white rounded-3xl shadow-sm p-4">
            ...
            <div className="space-y-1.5 text-xs mb-3"><div className="flex justify-between text-gray-400"><span>Current KM</span><span className="font-black text-gray-700">{(car.currentMileage||0).toLocaleString()} km</span></div><div className="flex justify-between text-gray-400"><span>Last Change</span><span className="font-black text-gray-700">{car.lastOilChangeDate?fmtD(car.lastOilChangeDate):'—'}</span></div><div className="flex justify-between text-gray-400"><span>Next at</span><span className="font-black text-gray-700">{(car.nextOilChangeMileage||0).toLocaleString()} km</span></div><div className={`flex justify-between font-black ${over?'text-rose-600':soon?'text-amber-600':'text-emerald-600'}`}><span>Remaining</span><span>{over?`🚨 ${Math.abs(km).toLocaleString()} km OVERDUE`:`${km.toLocaleString()} km to go`}</span></div></div>
            <div className="w-full bg-gray-100 rounded-full h-3 mb-3 overflow-hidden"><div className={`h-3 rounded-full transition-all ${over?'bg-gradient-to-r from-rose-400 to-pink-500':soon?'bg-gradient-to-r from-amber-400 to-orange-400':'bg-gradient-to-r from-emerald-400 to-teal-500'}`} style={{width:`${pct}%`}}/></div>
      ...
      {oilForm&&<OilChangeModal car={s.cars.find(c=>c.id===oilForm)} onClose={()=>setOilForm(null)} onSave={data=>{ const car=s.cars.find(c=>c.id===oilForm); d({t:'UPD_CAR',p:{...car,...data,currentMileage:+data.lastOilChangeMileage||car.currentMileage}}); d({t:'ADD_EXP',p:{id:uid(),carId:oilForm,type:'maintenance',amount:+data.oilChangeCost||0,date:data.lastOilChangeDate||today(),description:`Oil change – ${data.oilType||''}`,notes:'',createdAt:new Date().toISOString()}}); setOilForm(null) }}/>}
    </div>
  )
}
function OilChangeModal({ car, onClose, onSave }) {
  const [form,setForm]=useState({lastOilChangeDate:today(),lastOilChangeMileage:car?.currentMileage||'',nextOilChangeMileage:(car?.currentMileage||0)+5000,oilType:car?.oilType||'5W-30',garage:car?.garage||'',oilChangeCost:''}); const set=(k,v)=>setForm(f=>({...f,[k]:v}))
  return (<Modal title={`🛢️ Oil Change – ${car?.nickname}`} onClose={onClose} footer={<Btn onClick={()=>onSave(form)} className="w-full">💾 Save Oil Change</Btn>}><Fld label="Date"><Inp type="date" value={form.lastOilChangeDate} onChange={e=>set('lastOilChangeDate',e.target.value)}/></Fld><div className="grid grid-cols-2 gap-3"><Fld label="At KM"><Inp type="number" value={form.lastOilChangeMileage} onChange={e=>{set('lastOilChangeMileage',e.target.value);set('nextOilChangeMileage',(+e.target.value||0)+5000)}}/></Fld><Fld label="Next at KM"><Inp type="number" value={form.nextOilChangeMileage} onChange={e=>set('nextOilChangeMileage',e.target.value)}/></Fld></div><Fld label="Oil Type"><Inp value={form.oilType} onChange={e=>set('oilType',e.target.value)}/></Fld><Fld label="Garage"><Inp value={form.garage} onChange={e=>set('garage',e.target.value)}/></Fld><Fld label="Cost (QAR)"><Inp type="number" value={form.oilChangeCost} onChange={e=>set('oilChangeCost',e.target.value)}/></Fld></Modal>)
}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-amber-50/30 pb-28">
      ...
      <div className="px-4 py-4 space-y-5">
        {groups.map(g=>{ const grp=items.filter(g.fn); if(!grp.length) return null; return <div key={g.title}><p className="text-xs font-black text-gray-600 mb-2">{g.title} ({grp.length})</p><div className="space-y-2">{grp.map((item,i)=>{ const dd=item.days; const cls=dd<0?'bg-rose-50 border-rose-100':dd<=7?'bg-orange-50 border-orange-100':dd<=30?'bg-amber-50 border-amber-100':'bg-emerald-50 border-emerald-100'; const tc=dd<0?'text-rose-700':dd<=7?'text-orange-700':dd<=30?'text-amber-700':'text-emerald-700'; return <button key={i} onClick={()=>nav('car-form',{id:item.car.id})} className={`w-full flex justify-between items-center p-3.5 rounded-2xl border text-left active:scale-[0.98] transition-transform ${cls}`}><div><div className="text-sm font-black text-gray-800">{item.car.nickname}</div><div className="text-xs text-gray-500">{item.label}</div></div><div className="text-right"><div className={`text-sm font-black ${tc}`}>{dd<0?`${Math.abs(dd)}d ago`:dd===0?'Today!':dd<=30?`${dd}d`:'✅'}</div><div className="text-xs text-gray-400">{fmtD(item.date)}</div></div></button> })}</div></div> })}
...
function AccidentsScreen({ nav, openForCar }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-rose-50/30 pb-28">
      ...
      <div className="px-4 py-4 space-y-3">
        {s.accidents.length===0&&<div className="text-center py-16"><div className="text-5xl mb-3">🙏</div><p className="font-black text-gray-400">No accidents!</p><p className="text-sm text-gray-300 mt-1">Hopefully it stays that way!</p></div>}
        {s.accidents.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(acc=>{ const car=s.cars.find(c=>c.id===acc.carId); return <div key={acc.id} className="bg-white rounded-3xl shadow-sm p-4"><div className="flex justify-between items-start mb-2"><div><div className="font-bold text-gray-900">{car?.nickname||'Unknown Car'}</div><div className="text-xs text-gray-400">{fmtD(acc.date)}{acc.policeReportNumber?` · ${acc.policeReportNumber}`:''}</div></div><Badge status={acc.status} map={ACC_STATUS}/></div><p className="text-sm text-gray-600 mb-2">{acc.description}</p><div className="text-xs text-gray-400 mb-3 bg-gray-50 rounded-xl p-2">💰 Est: {fmt(acc.estimatedCost||0)} · Actual: {fmt(acc.actualCost||0)}</div><div className="flex gap-3"><button onClick={()=>{setEditing(acc);setShowForm(true)}} className="text-xs text-violet-600 font-black flex items-center gap-1"><Edit size={12}/>Edit / Photos</button><button onClick={()=>setConfirmDel(acc.id)} className="text-xs text-rose-400 font-black flex items-center gap-1 ml-2"><Trash2 size={12}/>Delete</button></div></div> })}
      </div>
      {showForm&&<AccidentForm cars={s.cars} existing={editing} initCarId={openForCar} onClose={()=>{setShowForm(false);setEditing(null)}} onSave={p=>{ d({t:editing?'UPD_ACC':'ADD_ACC',p}); if(!editing){const car=s.cars.find(c=>c.id===p.carId);if(car&&car.status!=='accident')d({t:'UPD_CAR',p:{...car,status:'accident'}})} setShowForm(false);setEditing(null) }}/>}
      {confirmDel&&<Confirm msg="Delete this accident record and its photos?" onOk={async()=>{ await delAcc(confirmDel);setConfirmDel(null) }} onCancel={()=>setConfirmDel(null)}/>}
    </div>
  )
}
function AccidentForm({ cars, existing, initCarId, onClose, onSave }) {
  const [form,setForm]=useState(existing||{id:uid(),carId:initCarId||'',date:today(),description:'',driverInvolved:'',policeReportNumber:'',insuranceClaimNumber:'',repairGarage:'',estimatedCost:'',actualCost:'',status:'open',notes:'',createdAt:new Date().toISOString()}); const set=(k,v)=>setForm(f=>({...f,[k]:v}))
  const save=()=>{ if(!form.carId||!form.description){alert('Please select a car and describe the accident');return} onSave({...form,estimatedCost:+form.estimatedCost||0,actualCost:+form.actualCost||0}) }
  return (<Modal title={existing?'✏️ Edit Accident':'🚨 New Accident'} onClose={onClose} footer={<Btn onClick={save} className="w-full">💾 Save Record</Btn>}><Fld label="Car *"><Sel value={form.carId} onChange={e=>set('carId',e.target.value)}><option value="">— Select Car —</option>{cars.map(c=><option key={c.id} value={c.id}>{CAR_STATUS[c.status]?.emoji} {c.nickname} ({c.plateNumber})</option>)}</Sel></Fld><div className="grid grid-cols-2 gap-3"><Fld label="Date"><Inp type="date" value={form.date} onChange={e=>set('date',e.target.value)}/></Fld><Fld label="Status"><Sel value={form.status} onChange={e=>set('status',e.target.value)}>{Object.entries(ACC_STATUS).map(([v,{label}])=><option key={v} value={v}>{label}</option>)}</Sel></Fld></div><Fld label="Description *"><Txa value={form.description} onChange={e=>set('description',e.target.value)} placeholder="What happened?"/></Fld><Fld label="Driver Involved"><Inp value={form.driverInvolved} onChange={e=>set('driverInvolved',e.target.value)}/></Fld><div className="grid grid-cols-2 gap-3"><Fld label="Police Report No."><Inp value={form.policeReportNumber} onChange={e=>set('policeReportNumber',e.target.value)}/></Fld><Fld label="Insurance Claim No."><Inp value={form.insuranceClaimNumber} onChange={e=>set('insuranceClaimNumber',e.target.value)}/></Fld></div><Fld label="Repair Garage"><Inp value={form.repairGarage} onChange={e=>set('repairGarage',e.target.value)}/></Fld><div className="grid grid-cols-2 gap-3"><Fld label="Est. Cost (QAR)"><Inp type="number" value={form.estimatedCost} onChange={e=>set('estimatedCost',e.target.value)}/></Fld><Fld label="Actual Cost (QAR)"><Inp type="number" value={form.actualCost} onChange={e=>set('actualCost',e.target.value)}/></Fld></div><Fld label="Notes"><Txa value={form.notes} onChange={e=>set('notes',e.target.value)}/></Fld>{form.carId&&<div className="pt-2 border-t border-gray-100 mt-2"><MediaSection title="📷 Accident Photos" ownerType="accident" ownerId={form.id} category="photos"/><MediaSection title="📄 Police Report" ownerType="accident" ownerId={form.id} category="police_report" single/><MediaSection title="🛡️ Insurance Docs" ownerType="accident" ownerId={form.id} category="insurance_docs" single/></div>}</Modal>)
}
...
function FinanceScreen({ nav, openExpense, preCarId }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-amber-50/30 pb-28">
      ...
      {showForm&&<ExpenseForm cars={s.cars} existing={editing} initCarId={preCarId||(carF!=='all'?carF:'')} onClose={()=>{setShowForm(false);setEditing(null)}} onSave={p=>{d({t:editing?'UPD_EXP':'ADD_EXP',p});setShowForm(false);setEditing(null)}}/>}
      {confirmDel&&<Confirm msg="Delete this expense?" onOk={()=>{d({t:'DEL_EXP',p:confirmDel});setConfirmDel(null)}} onCancel={()=>setConfirmDel(null)}/>}
    </div>
  )
}
function ExpenseForm({ cars, existing, initCarId, onClose, onSave }) {
  ...
  const save=()=>{ if(!form.carId||!form.amount){alert('Please select a car and enter an amount');return} onSave({...form,amount:+form.amount}) }
  return (<Modal title={existing?'✏️ Edit Expense':'💸 Add Expense'} onClose={onClose} footer={<Btn onClick={save} className="w-full">{existing?'💾 Save':'➕ Add Expense'}</Btn>}><Fld label="Car *"><Sel value={form.carId} onChange={e=>set('carId',e.target.value)}><option value="">— Select Car —</option>{cars.map(c=><option key={c.id} value={c.id}>{CAR_STATUS[c.status]?.emoji} {c.nickname} ({c.plateNumber})</option>)}</Sel></Fld><div className="grid grid-cols-2 gap-3"><Fld label="Type"><Sel value={form.type} onChange={e=>set('type',e.target.value)}>{EXP_TYPES.map(t=><option key={t} value={t}>{t.charAt(0).toUpperCase()+t.slice(1)}</option>)}</Sel></Fld><Fld label="Amount (QAR)"><Inp type="number" value={form.amount} onChange={e=>set('amount',e.target.value)}/></Fld></div><Fld label="Date"><Inp type="date" value={form.date} onChange={e=>set('date',e.target.value)}/></Fld><Fld label="Description"><Inp value={form.description} onChange={e=>set('description',e.target.value)} placeholder="What was this for?"/></Fld><Fld label="Notes"><Txa value={form.notes} onChange={e=>set('notes',e.target.value)}/></Fld></Modal>)
}
...
function MoreScreen({ nav }) {
  return (
    <div className="min-h-screen bg-gray-50 pb-28">
      <Header title="More"/>
      <div className="px-4 py-4 space-y-3">
        {[{em:'🔧',label:'Maintenance & Oil',desc:'Track oil changes per car',fn:()=>nav('maintenance',{}),grad:'from-amber-400 to-orange-500'},{em:'🔔',label:'Renewals & Documents',desc:'Insurance, registration, inspection',fn:()=>nav('renewals',{}),grad:'from-blue-500 to-cyan-400'},{em:'🚨',label:'Accident Records',desc:'Accidents, repairs, insurance claims',fn:()=>nav('accidents',{}),grad:'from-rose-400 to-pink-500'},{em:'⚙️',label:'Settings & Backup',desc:'Export, import and cloud sync',fn:()=>nav('settings',{}),grad:'from-gray-400 to-gray-500'}].map(item=>(
...
function SettingsScreen({ nav }) {
  ...
  const exportJSON = async () => {
    ...
    try {
      ...
      for (const m of s.media) {
        const v = mediaCache.get(m.id) || localStorage.getItem(MEDIA_PFX + m.id)
        if (v) mediaData[m.id] = v
      }
      const a = document.createElement('a')
      a.href = URL.createObjectURL(new Blob([JSON.stringify({ ...s, mediaData })], { type:'application/json' }))
      a.download = `fleet-backup-${today()}.json`; a.click(); URL.revokeObjectURL(a.href)
    } catch(e) { alert('Export failed') }
    setBusy(false)
  }

  const exportCSV = () => {
    const rows = [['Customer','Car','Plate','Start','End','Days','Rate','Total','Paid','Balance','Status'], ...s.rentals.map(r=>{ const car=s.cars.find(c=>c.id===r.carId); const dur=calcDur(r.startDate,r.endDate); const total=r.rateAmount*dur; return [r.customerName,car?.nickname||'',car?.plateNumber||'',r.startDate,r.endDate,dur,r.rateAmount,total,r.paymentReceived||0,total-(r.paymentReceived||0),r.status] })]
  ...
  const importJSON = e => {
    ...
    reader.onload = ev => {
      try {
        const { mediaData, ...rest } = JSON.parse(ev.target.result)
        if (mediaData) { Object.entries(mediaData).forEach(([id,val]) => { if(val) { mediaCache.set(id,val); try { localStorage.setItem(MEDIA_PFX+id,val) } catch(e){} } }) }
        d({ t:'LOAD', p:rest }); alert('Data imported! ✅')
  ...
  const handleReset = () => {
    ...
    const keys = []; for (let i=0;i<localStorage.length;i++) { const k=localStorage.key(i); if(k?.startsWith(MEDIA_PFX)) keys.push(k) }
    keys.forEach(k=>localStorage.removeItem(k)); mediaCache.clear()
    d({ t:'RESET' }); setConfirmReset(false)
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-28">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <CloudSync />
        <SCard title="💾 Export"><div className="space-y-2.5"><Btn onClick={exportJSON} v="outline" disabled={busy} className="w-full flex items-center justify-center gap-2"><Download size={16}/>{busy?'Packing…':'Export Full Backup (JSON)'}</Btn><Btn onClick={exportCSV} v="outline" className="w-full flex items-center justify-center gap-2"><FileText size={16}/>Export Rentals (CSV)</Btn></div><p className="text-xs text-gray-400 mt-2 text-center">Full backup includes all photos 📷</p></SCard>
        <SCard title="📂 Import"><label className="block"><div className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-black border-2 border-dashed border-gray-200 bg-white text-gray-500 rounded-2xl cursor-pointer active:bg-gray-50"><Upload size={16}/>{busy?'Importing…':'Import Backup (JSON)'}</div><input type="file" accept=".json" className="hidden" onChange={importJSON} disabled={busy}/></label><p className="text-xs text-gray-400 mt-2 text-center">Replaces all current data</p></SCard>
        <SCard title="🔥 Danger Zone"><Btn onClick={()=>setConfirmReset(true)} v="danger" className="w-full flex items-center justify-center gap-2"><RefreshCw size={16}/>Reset to Demo Data</Btn><p className="text-xs text-gray-400 mt-2 text-center">Resets everything including photos</p></SCard>
        <p className="text-center text-xs text-gray-300 py-2">🚗 Fleet Manager v2.0 · Built for Ahmed</p>
      </div>
      {confirmReset&&<Confirm msg="Reset ALL data to demo data? This cannot be undone!" onOk={handleReset} onCancel={()=>setConfirmReset(false)}/>}
    </div>
  )
}
...
export default function App() {
  const [screen,setScreen]=useState('home'); const [params,setParams]=useState({}); const [tab,setTab]=useState('home')
  const nav=useCallback((scr,p={})=>{ setScreen(scr);setParams(p);if(['home','cars','rentals','finance','more'].includes(scr))setTab(scr);window.scrollTo(0,0) },[])
  ...
  const render=()=>{
    switch(screen){
      ...
      case 'car-form':      return <CarForm carId={params.id} nav={nav}/>
  ...
  return (
    <Store>
      <div className="max-w-md mx-auto relative min-h-screen">{render()}<BottomNav tab={tab} setTab={switchTab}/></div>
    </Store>
  )
}
